...
@implementation NSPingFoundation
...
- (void)didFailWithError:(NSError *)error {
    id<NSPingFoundationDelegate>  strongDelegate;
    ...
    if ((strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(NSPingFoundation:didFailWithError:)])
    {
        [strongDelegate NSPingFoundation:self didFailWithError:error];
    }
}
...
- (void)sendPingWithData:(NSData *)data {
    ...
    if ( (bytesSent > 0) && (((NSUInteger) bytesSent) == packet.length) ) {
        ...
        if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(NSPingFoundation:didSendPacket:sequenceNumber:)] ) {
            [strongDelegate NSPingFoundation:self didSendPacket:packet sequenceNumber:self.nextSequenceNumber];
        }
    } else {
        NSError *   error;
        ...
        if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(NSPingFoundation:didFailToSendPacket:sequenceNumber:error:)] ) {
            [strongDelegate NSPingFoundation:self didFailToSendPacket:packet sequenceNumber:self.nextSequenceNumber error:error];
        }
    }
...
- (void)readData {
    ...
    if (bytesRead > 0) {
        ...
        if ( [self validatePingResponsePacket:packet sequenceNumber:&sequenceNumber] ) {
            if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(NSPingFoundation:didReceivePingResponsePacket:sequenceNumber:)] ) {
                [strongDelegate NSPingFoundation:self didReceivePingResponsePacket:packet sequenceNumber:sequenceNumber];
            }
        } else {
            if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(NSPingFoundation:didReceiveUnexpectedPacket:)] ) {
                [strongDelegate NSPingFoundation:self didReceiveUnexpectedPacket:packet];
            }
        }
    } else {
...
static void SocketReadCallback(CFSocketRef s, CFSocketCallBackType type, CFDataRef address, const void *data, void *info) {
    ...
    NSPingFoundation *    obj;
    
    obj = (__bridge NSPingFoundation *) info;
...
- (void)startWithHostAddress
{
    ...
    if (err != 0) {
        ...
        strongDelegate = self.delegate;
        if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(NSPingFoundation:didStartWithAddress:)] ) {
            [strongDelegate NSPingFoundation:self didStartWithAddress:self.hostAddress];
        }
    }
}
        ...
        for (NSData * address in addresses) {
            ...
            if ( address.length >= sizeof(struct sockaddr) ) {
                switch (addrPtr->sa_family) {
                    case AF_INET: {
                        if (self.addressStyle != NSPingFoundationAddressStyleICMPv6) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
                    case AF_INET6: {
                        if (self.addressStyle != NSPingFoundationAddressStyleICMPv4) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
...
static void HostResolveCallback(CFHostRef theHost, CFHostInfoType typeInfo, const CFStreamError *error, void *info) {
    ...
    if (([obj isKindOfClass:[NSPingFoundation class]] && typeInfo == kCFHostAddresses))
    {
