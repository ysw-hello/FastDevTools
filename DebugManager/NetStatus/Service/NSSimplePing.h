...
typedef NS_ENUM(NSInteger, NSSimplePingAddressStyle) {
    NSSimplePingAddressStyleAny,          ///< Use the first IPv4 or IPv6 address found; the default.
    NSSimplePingAddressStyleICMPv4,       ///< Use the first IPv4 address found.
    NSSimplePingAddressStyleICMPv6        ///< Use the first IPv6 address found.
};



@interface NSSimplePing : NSObject
...
@property (nonatomic, weak, readwrite, nullable) id<NSSimplePingDelegate> delegate;

/*! Controls the IP address version used by the object.
 *  \details You should set this value before starting the object.
 */

@property (nonatomic, assign, readwrite) NSSimplePingAddressStyle addressStyle;
...
@protocol NSSimplePingDelegate <NSObject>
